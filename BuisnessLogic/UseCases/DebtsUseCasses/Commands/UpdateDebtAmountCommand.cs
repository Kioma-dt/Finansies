...
{
    public sealed record UpdateDebtAmountCommand(Guid UserId,
        Guid DebtId,
        DateTime Date)
        : IRequest;

    public class UpdateDebtCommandAmountHandler(IDebtRepository debtRepository,
        IDebtInterestCalculatorProvider debtInterestCalculatorProvider)
        : IRequestHandler<UpdateDebtAmountCommand>
    {
        public async Task Handle(UpdateDebtAmountCommand request, CancellationToken cancellationToken)
        {
